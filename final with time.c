...
void gather_data_linkedlist2(struct Node* node)
{
    for (int i = 0; i < 29; i++)
    {
  	    node->stu.name[i]='a';
...
void gather_data_array2(student* element, int no_of_elements) 
  {
...
void insert_linkedlist2(int index)
{
    struct Node* new_node = (struct Node*)malloc(sizeof(struct Node));
   gather_data_linkedlist2(new_node);
	if (head == NULL)
	{
...
int main()
{
	...
	{
		insert_linkedlist2(size_linkedlist+1);
